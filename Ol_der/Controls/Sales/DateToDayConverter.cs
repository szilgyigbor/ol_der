    ...
    public class DateToDayConverter : IValueConverter
    {
        public object Convert(object value, Type targetType, object parameter, CultureInfo culture)
        {
            if (value is CollectionViewGroup group)
            {
                ...
                return $"{formattedDate}: (Készpénz: {totalCash:0}.-Ft,   RN: {totalRN:0}.-Ft,   Kártya: {totalCard:0}.-Ft,   Átutalás: {totalTransfer:0}.-Ft   Napi teljes bevétel: {totalCash + totalCard + totalRN + totalTransfer:0}.-Ft)";
            }
            return null;
        }
